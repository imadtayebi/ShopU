...
public class EliminarCuenta extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ps = conn.prepareStatement("SELECT email,clave FROM USUARIOS where email='" + email + "'");
            //ps.execute();
            ps.executeQuery();
            ...
            if (pass.equals(clave)) {
                ps = conn.prepareStatement("DELETE FROM USUARIOS where email='" + email + "'");
                filasAfectadas = ps.executeUpdate();
            } else {
                System.out.println("PASS ERRONEA");
            }

            // Preparar la sentencia SQL a realizar
            // Ejecutar instrucción SQL y guardar resultado en msg
            if (filasAfectadas > 0) {
                msg = "<p>OK: Delete realizado correctamente</p>";
            } else {
